...
public class TicTacToe implements State {
    ...
    public static TicTacToe start(int dim) {
        return new TicTacToe((short) dim, (short) dim);
    }

    public static TicTacToe start(int dim, int needed) {
        return new TicTacToe((short) dim, (short) needed);
    }
    ...
    public TicTacToe(short dims, short needed) {
        assert dims >= needed;
        this.needed = needed;
        this.agent = 1;
        ...
        for (short i = 0; i < board.length; i++) {
            if (board[i] == 0) {
                try{
					...
					System.out.println(remaining+ " "+idx);
				}
            }
        }
